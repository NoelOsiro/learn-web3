...
export default async function CollectionsPage({ searchParams }: PageProps) {
  ...
  const totalVolume = collections.reduce((a: number, c: any) => a + c.quantity, 0);
  const totalGrossValue = collections.reduce((a: number, c: any) => a + c.grossAmount, 0);
  const totalNetValue = collections.reduce((a: number, c: any) => a + c.netAmount, 0);
